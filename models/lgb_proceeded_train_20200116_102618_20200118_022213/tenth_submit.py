...
def preprocess(train: pd.DataFrame,
    ...
    train = pd.merge(
        train,
        event_cluster,
        left_on='event_id', right_index=True).sort_index()
    ...
    compiled_train, compiled_test = mutual_feature_preprocess(
...
def feature_preprocess(df):
    ...
    def count_accuracy_label_encoder(x):
        if x == 0:
            y = 0
        elif x > 0 and x <= 0.34:
            y = 1
    ...
    def mean_accuracy_group_label_encoder(x):
        if x == 0:
            y = 0
        elif x > 0 and x < 1.0:
            y = 1
    ...
    def bins_count_action_label_encoder(x):
        if x < 1.0:
            y = 0
        elif x >= 1.0 and x < 2.0:
            y = 1
    ...
    df['frequency_label'] = df.frequency.apply(lambda x: frequency_label_encoder(np.log1p(x)))
    df['count_action_label'] = df.count_actions.apply(lambda x: bins_count_action_label_encoder(np.log1p(x)))
    df['count_accuracy_label'] = df.count_accuracy.apply(lambda x: count_accuracy_label_encoder(x))
    df['mean_accuracy_group_label'] = df.mean_accuracy_group.apply(lambda x: mean_accuracy_group_label_encoder(x))
    ...
    df['sum_event_code_count'] = df[[2050, 4100, 4230, 5000, 4235, 2060, 4110, 5010, 2070, 2075, 2080, 2081, 2083, 3110, 4010, 3120, 3121, 4020, 4021, 
                                    4022, 4025, 4030, 4031, 3010, 4035, 4040, 3020, 3021, 4045, 2000, 2010, 2020, 4070, 2025, 2030, 4080, 2035, 
                                    2040, 4090, 4220, 4095, 4050]].sum(axis=1)
...
def mutual_feature_preprocess(train, test):

    def list_label_encoder(train, test, columns: list):
        le = LabelEncoder()
    ...
    def frequency_encoder(train, test, label_name):
    ...
    train, test, label = list_label_encoder(train, test, ['description_val'])
    train, test = frequency_encoder(train, test, label)

    train, test, label = list_label_encoder(train, test, ['frequency_label', 'description_val'])
    train, test = frequency_encoder(train, test, label)
    train, test, label = list_label_encoder(train, test, ['mean_accuracy_group_label', 'description_val'])
    train, test = frequency_encoder(train, test, label)
    train, test, label = list_label_encoder(train, test, ['count_action_label', 'description_val'])
    train, test = frequency_encoder(train, test, label)

    train, test, label = list_label_encoder(train, test, ['session_title', 'description_val'])
    train, test = frequency_encoder(train, test, label)

    train, test, label = list_label_encoder(train, test, ['session_title', 'count_action_label'])
    train, test = frequency_encoder(train, test, label)

    train, test, label = list_label_encoder(train, test, ['session_title', 'count_accuracy_label'])
    train, test = frequency_encoder(train, test, label)

    train, test, label = list_label_encoder(train, test, ['session_title', 'mean_accuracy_group_label'])
    train, test = frequency_encoder(train, test, label)
...
def train_main(train_df, test_df):
    ...
    seed_list = [77, 5, 8, 16, 42, 10, 777, 98, 356, 56, 1, 212, 444, 612, 593]
    ...
    # ctest_pred.to_csv(f'models/{args.name}/check_cv.csv')
