# val_loss(seed=3): 0.5599113457824938
...
EVENT_CODE_LIST  = [2050, 4100, 4230, 5000, 4235, 2060, 4110, 5010, 2070, 2075, 2080,
                    2081, 2083, 3110, 4010, 3120, 3121, 4020, 4021,
                    4022, 4025, 4030, 4031, 3010, 4035, 4040, 3020, 3021, 4045, 2000,
                    2010, 2020, 4070, 2025, 2030, 4080, 2035,
                    2040, 4090, 4220, 4095]
...
DROP_FEATURES = ['frequency_label', 'count_action_label', 'count_accuracy_label',
                 'sum_event_code_count',
                 'info_0', 'info_4', 'info_5', 'info_9']


def preprocess_features_main(train, test):
...
class PreprocessFeatures:
    ...
    def process(self, df):
        ...
        df['sum_event_code_count'] = df[EVENT_CODE_LIST].sum(axis=1)
...
class TargetEncoding:
    ...
    def process(self, x_train, y_train, x_val,
                test_x, columns: list, seed: int = 77, val_x=None):
        """
        ...
        for column in columns:
            ...
            if val_x is not None:
                val_x[f'target_{column}'] = val_x[column].map(target_mean)
        ...
        if val_x is not None:
            return x_train, x_val, test_x, val_x
        else:
