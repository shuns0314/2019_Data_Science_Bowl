...
def predict_main(test_df: pd.DataFrame) -> pd.DataFrame:
    model_name_list = glob.glob(MODEL_PATH)
...
def main():
    ...
    print('--preprocess--')
    _, compiled_test = preprocess_features_main(
        compiled_train, compiled_test)
    print(compiled_test.shape)

    print('--predict--')
    pred_df = predict_main(compiled_test)
