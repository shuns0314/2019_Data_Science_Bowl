...
def lgb_regression(train_df: pd.DataFrame, test_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    x_1 = x.drop(MODEL_1_DROP_LIST, axis=1)
    x_1 = x_1.drop('session_title', axis=1)
    x_2 = x.drop(x.loc[:, x.columns.str.contains('args')].columns, axis=1)
    x_2 = x_2.drop(x_2.loc[:, x_2.columns.str.contains('info')].columns, axis=1)
    ...
    if test_df is not None:
        ...
        test_x_1 = test_x.drop(MODEL_1_DROP_LIST, axis=1)
        test_x_1 = test_x_1.drop('session_title', axis=1)
        test_x_2 = test_x.drop(test_x.loc[:, test_x.columns.str.contains('args')].columns, axis=1)
        test_x_2 = test_x_2.drop(test_x_2.loc[:, test_x_2.columns.str.contains('info')].columns, axis=1)
    ...
    for fold_ind, (train_ind, test_ind) in enumerate(
        ...
        x_1_train = x_1.iloc[train_ind]
        x_2_train = x_2.iloc[train_ind]
        y_train = y.iloc[train_ind]
        x_1_test = x_1.iloc[test_ind]
        x_2_test = x_2.iloc[test_ind]
        y_test = y.iloc[test_ind]

        # model_1 Simple model
        lgb_train_1 = lgb.Dataset(x_1_train, y_train)
        lgb_val_1 = lgb.Dataset(x_1_test, y_test, reference=lgb_train_1)
        model_1 = lgb.train(params=lgb_params,
                            train_set=lgb_train_1,
                            valid_sets=lgb_val_1,
                            feval=lgb_qwk)
        y_pred_1 = model_1.predict(x_1_test, num_iteration=model_1.best_iteration)
        all_importance.append(pd.DataFrame(model_1.feature_importance('gain'),
                                           index=x_1_train.columns))
        
        # model_2 Complex model
        lgb_train_2 = lgb.Dataset(x_2_train, y_train)
        lgb_val_2 = lgb.Dataset(x_2_test, y_test, reference=lgb_train_2)
        model_2 = lgb.train(params=lgb_params,
                            train_set=lgb_train_2,
                            valid_sets=lgb_val_2,
                            feval=lgb_qwk)
        y_pred_2 = model_2.predict(x_2_test, num_iteration=model_2.best_iteration)
        all_importance.append(pd.DataFrame(model_2.feature_importance('gain'),
                                           index=x_2_train.columns))
        
        y_pred = 0.2 * y_pred_1 + 0.8 * y_pred_2
        # print(test_ind)
        ...
        if test_df is not None:
            test_pred_1 = model_1.predict(test_x_1, num_iteration=model_1.best_iteration)
            test_pred_2 = model_2.predict(test_x_2, num_iteration=model_2.best_iteration)

            test_pred = 0.5 * test_pred_1 + 0.5 * test_pred_1

            total_test_pred[:, fold_ind] = test_pred

    all_importance = pd.concat(all_importance, axis=1, sort=True)
    params = {
        'threshold_0': 1.12,
        'threshold_1': 1.62,
        'threshold_2': 2.20
    ...
    if test_df is None:
        return model_1, all_importance, loss
    else:
        return model_1, all_importance, pd.DataFrame(total_test_pred), loss
