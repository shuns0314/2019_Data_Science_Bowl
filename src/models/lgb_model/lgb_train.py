...
from src.models.post_process import threshold
from src.models.closs_validation import stratified_group_k_fold
...
def main():
    ...
    train_df = pd.read_csv(f"data/transformation/{args.train_csv}.csv", index_col=0)
    train_df.columns = train_df.columns.str.replace(',', '')
    test_df = pd.read_csv(f"data/transformation/{args.test_csv}.csv", index_col=0)
...
def train_and_predict(train_df: pd.DataFrame, val_df: pd.DataFrame,
    ...
    for fold_ind, (train_ind, inner_val_ind) in enumerate(
        ...
        if not os.path.exists(f"{path}/lgb_output"):
            os.mkdir(f"{path}/lgb_output")
        with open(f"{path}/lgb_output/"+f"{fold_ind}_{trial_uuid}.pkl", "wb") as fp:
